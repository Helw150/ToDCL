...
def test_model_seq2seq_ADAPTER(
    ...
    for t in range(range_adpt):
        print(f"Task {t}")
        for idx_b, batch in tqdm(enumerate(test_loader), total=len(test_loader)):
            task_id = model.task_list_seen[t]
            ppl_batch = model.compute_PPL(
                batch, task_id=task_id, device=device
            )  ## one value per batch
    ...
    for pred_task_id, task_loader in tqdm(
        ...
        print(f"Task Id: {pred_task_id}")
        self.model.train_adapter(pred_task_id)
        self.model.set_active_adapters(pred_task_id)
